...
parameters_cc getCcParameters() {
	...
	string filename = "./config/inParams.json";
	...
	while (1) {
		...
		while (true) {
			...
			if (key == 's') {	// sで現在の状態を保存し出力
				...
				cv::imshow("Tuned img", ret.projImg);
				cv::imwrite("./results/projImg_threads_tuned.bmp", ret.projImg);
...
void make_directionalImg_filtered(
	...
	cv::imwrite("./results/projImg_threads_directional_filtered.bmp", finalCanvas);
	cv::waitKey(0);
}
...
int main() {
	...
	//静止画像からArUcoマーカーを検出
	result_test_arMarkerDetect out = test_arMarkerDetect(dictionary, p_camMat, "./data/testImg.bmp");
	...
	cv::Mat dImg = cv::imread("./data/projImg_threads.bmp");
